...
from xgnn.cache.utils import CSRTopo, init_p2p
from xgnn.cache.feature import Feature
...
class NeighborSampler(object):
    ...
    def sample_blocks(self, seeds):
        ...
        #print("sample time: ", (time.time()-start)*1000, "ms")    
        ...
        #print("load_feat time: ", (time.time()-start)*1000, "ms")    
        if self.load_feat:
...
def evaluate_with_subgraph(model, nfeat, dataloader, device):
    all_labels = []
    all_output = []
    with th.no_grad():
        for blocks in tqdm.tqdm(dataloader):
...
def run(args, device, data):
    ...
    # Unpack data
    train_nid, val_nid, test_nid, in_feats, n_classes, nfeat, g = data
    # train_nid, val_nid, test_nid, in_feats, n_classes, g = data
    # print("g:",g.ndata)
    # sg=g.local_partition
    # print("g.local_partition:",sg.ndata)
    ...
    for epoch in range(args.num_epochs):
        ...
        with model.join():
            for step, blocks in enumerate(dataloader):
                ...
                # The nodes for input lies at the LHS side of the first block.
                # The nodes for output lies at the RHS side of the last block.
                # print(th.cuda.max_memory_allocated())
                
                input_nodes = blocks[0].srcdata[dgl.NID]
                # print("loginfo step:", step)
                batch_inputs = nfeat[input_nodes].to(device)              
                # batch_inputs = blocks[0].srcdata["feat"]
                feature_time += time.time() - tic_step
                batch_labels = blocks[-1].dstdata["labels"]
                batch_labels = batch_labels.long()
        ...
        toc = time.time()
        logger.info(
            "Part {}, Epoch Time(s): {:.4f}, sample+data_copy: {:.4f}, forward: {:.4f}, feature: {:.4f}, backward: {:.4f}, update: {:.4f}, #seeds: {}, #inputs: {}".format(
                g.rank(),
                toc - tic,
                sample_time,
                forward_time,
        ...
        if False:
            ...
            val_acc = evaluate_with_subgraph(model, nfeat, val_dataloader, device)
            logger.info(
...
def load_features(part_config, part_id):
    ...
    part_files = part_metadata["part-0"]
    assert "part_feats.dgl" in part_files, "the partition does not contain node features."
    part_feats = load_tensors(relative_to_config(part_files["part_feats.dgl"]))
...
def main(args):
    ...
    feat = g.ndata['feat'][range(g.number_of_nodes())]
    csr_topo = CSRTopo(th.stack(g.local_partition.edges('uv'))) 
    print("CSRTopo construction finished")
    device_list = list(range(args.num_gpus))

    # nfeat = Feature(rank = g.rank(), device_list=device_list,
    #                             device_cache_size="200M", cache_policy="device_replicate", 
    #                            csr_topo=csr_topo)

    init_p2p(device_list)
    nfeat = Feature(rank = 0, device_list=device_list,
                                device_cache_size="200M", cache_policy="p2p_clique_replicate", 
                               csr_topo=csr_topo)   
    
    nfeat.from_cpu_tensor(feat)

    data = train_nid, val_nid, test_nid, in_feats, n_classes, nfeat, g
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fan_out", type=str, default="20,15,10")
