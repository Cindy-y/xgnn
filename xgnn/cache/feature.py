import torch
from .utils import reindex_feature, CSRTopo, parse_size
from .shared_tensor import  Topo, SharedTensor, SharedTensorConfig
from typing import List
import numpy as np
from torch._C import device

__all__ = ["Featrue", "DeviceConfig"]
...
class Feature(object):
    ...
    def clique_device_symmetry_check(self):
        '''if device_replicate, return ture, else warning'''
    ...
    def from_cpu_tensor(self, cpu_tensor: torch.Tensor):
        ...
        if cache_part.shape[0] > 0 and self.cache_policy == "device_replicate":
            for device in self.device_list:
                shard_tensor = SharedTensor(self.rank, SharedTensorConfig({}))
        ...
        # 构建CPU Tensor
        if self.cpu_part.numel() > 0:                       ## cpu_part.numel()计算该tensor中所有的张量元素数量
            if self.cache_policy == "device_replicate":
                shard_tensor = self.device_tensor_list.get(
                    self.rank, None) or SharedTensor(self.rank,
                                                    SharedTensorConfig({}))
                ...
                shard_tensor = self.clique_tensor_list.get(
                    clique_id, None) or SharedTensor(self.rank,
                                                    SharedTensorConfig({}))
                shard_tensor.append(self.cpu_part, -1)
    ...
    def share_ipc(self):
        ...
        Returns:
            tuples: ipc handles for SharedTensor and torch.Tensor and python native objects
        """
    ...
    def from_gpu_ipc_handle_dict(self, gpu_ipc_handle_dict, cpu_tensor):
        if self.cache_policy == "device_replicate":
            ipc_handle = gpu_ipc_handle_dict.get(
                self.rank, []), cpu_tensor, SharedTensorConfig({})
            shard_tensor = SharedTensor.new_from_share_ipc(
            ...
            clique_id = self.topo.get_clique_id(self.rank)
            ipc_handle = gpu_ipc_handle_dict.get(
                clique_id, []), cpu_tensor, SharedTensorConfig({})
            shard_tensor = SharedTensor.new_from_share_ipc(
                ipc_handle, self.rank)
            self.clique_tensor_list[clique_id] = shard_tensor
